...
if [ ! -f "$wrapper_jar" ]; then
    ...
    if command -v curl >/dev/null 2>&1; then
        curl -fL --retry 3 --retry-delay 2 -o "$wrapper_jar" "$wrapper_url" || die "Failed to download Gradle wrapper JAR from $wrapper_url"
    elif command -v wget >/dev/null 2>&1; then
        wget -O "$wrapper_jar" "$wrapper_url" || die "Failed to download Gradle wrapper JAR from $wrapper_url"
    else
        die "Cannot find curl or wget to download Gradle wrapper JAR from $wrapper_url"
    fi
fi
